...
for (int i = 1; i <= data.getRowNumbers(); i++) {
	...
	WebUI.setText(findTestObject("Bán Hàng/TaoDonHang/LuoiDanhSach/num_SoLuong", ["rowIndex" : 1]), soLuong)
	WebUI.delay(1)
...
for (int i = 0; i < listRows.size(); i++) {
	...
	// Lấy các element theo dòng
	WebElement inputDonGia = WebUI.findWebElement(findTestObject('Bán Hàng/num_DonGia', [('rowIndex') : row]), 10)
	WebElement inputSoLuong = WebUI.findWebElement(findTestObject('Bán Hàng/TaoDonHang/LuoiDanhSach/num_SoLuong', [('rowIndex') : row]), 10)
	WebElement thanhTienElement = WebUI.findWebElement(findTestObject('Bán Hàng/col_DanhSachThanhTien', [('rowIndex') : row]), 10)

	// Lấy giá trị text/value
	...
	// Chuyển sang kiểu số để tính toán
	...
	// So sánh (cho phép sai số 1 đơn vị do làm tròn)
...
//================================================
// 4. KIỂM TRA TỔNG CỘNG CUỐI CÙNG
//================================================
...
double parseMoney(String text) {
	...
	// Xóa tất cả ký tự không phải số, trừ dấu chấm phân cách thập phân nếu có
	// Ở đây xóa hết dấu phẩy, dấu ₫ và khoảng trắng
	String cleanText = text.replace(',', '').replace('.', '').replace('₫', '').trim()
	try {
		return Double.parseDouble(cleanText)
	} catch (Exception e) {
		println "Lỗi parse tiền: " + text
		return 0
	}
}